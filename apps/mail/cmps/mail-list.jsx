    ...
    return (
        <ul className="mail-list clean-list">
            {mails.map(mail => <li onDoubleClick={() => { openDetails(mail) }} className={`mail-list-item ${mail.isRead ? 'is-read' : ''}`} key={mail.id}>
                ...
                <button className="read-unread-btn" onClick={() => { onToggle.onToggleIsRead(mail.id) }}>
                    {mail.isRead ? (
                ...
                <button className="delete-btn" onClick={() => { onDeleteMail(mail.id, mail.folder) }}><img src="assets/img/mailIcons/asset50.png" /></button>
                <span className="sentAt">{ `${new Date(mail.sentAt).toLocaleTimeString()} ${new Date(mail.sentAt).toLocaleDateString()}`
                }</span>
            </li>)}
            {areMailDetailsOpen && (
